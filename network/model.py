...
from .blocks import BlockDown, Attention, Block, AdaptiveInstanceNorm
import math
import sys
import os
...
class Embedder(nn.Module):
    def __init__(self, in_height):
        ...
        #in 6*224*224
        self.pad = nn.ZeroPad2d(16) #out 6*256*256
        self.resDown1 = BlockDown(6, 64) #out 64*128*128
        self.resDown2 = BlockDown(64, 128) #out 128*64*64
        self.resDown3 = BlockDown(128, 256) #out 256*32*32
        self.self_att = Attention(256) #out 256*32*32
        self.resDown4 = BlockDown(256, 512) #out 515*16*16
        self.resDown5 = BlockDown(512, 512) #out 512*8*8
        self.resDown6 = BlockDown(512, 512) #out 512*4*4
...
class Generator(nn.Module):
    ...
    def __init__(self, in_height, finetuning=False, e_finetuning=None):
        ...
        self.pad = nn.ZeroPad2d(16) #out 3*256*256
        
        #Down
        self.resDown1 = BlockDown(3, 64) #out 64*128*128
        ...
        self.self_att_Down = Attention(256) #out 256*32*32
        
        self.resDown4 = BlockDown(256, 512) #out 512*16*16
        self.in4 = nn.InstanceNorm2d(512, affine=True)
        
        #Res
        #in 512*16*16
        self.res1 = Block(512, 512, 512)
        self.res2 = Block(512, 512, 512)
        self.res3 = Block(512, 512, 512)
        self.res4 = Block(512, 512, 512)
    
        #out 512*16*16
        
        #Up
        #in 512*16*16
        self.resUp1 = Block(512, 256, 512) #out 256*32*32
        self.resUp2 = Block(256, 128, 512) #out 128*64*64
        
        self.self_att_Up = Attention(128) #out 128*64*64

        self.resUp3 = Block(128, 64, 512) #out 64*128*128
        self.resUp4 = Block(64, 32, 512) #out 3*224*224
        self.in5 = AdaptiveInstanceNorm(32, 512)
    ...
    def forward(self, y, e):
        ...
        #Residual
        out = self.res1(out, ehat)
        out = self.res2(out, ehat)
        out = self.res3(out, ehat)
        out = self.res4(out, ehat)
 
        
        
        #Decoding
        out = self.resUp1(out, ehat)
        
        out = self.resUp2(out, ehat)
        
        out = self.self_att_Up(out)

        out = self.resUp3(out, ehat)
        
        out = self.resUp4(out, ehat)
        
        out = self.in5(out,ehat)
...
class Discriminator(nn.Module):
    def __init__(self, num_videos, path_to_Wi, finetuning=False, e_finetuning=None):
        ...
        self.resDown5 = BlockDown(512, 512) #out 512*8*8
        self.resDown6 = BlockDown(512, 512) #out 512*4*4
        self.res = Block(512,512, which_in=None, upsample=None) #out 512*4*4
        self.sum_pooling = nn.AdaptiveAvgPool2d((1,1)) #out 512*1*1
